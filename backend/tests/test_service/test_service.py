...
async def test_get_services_by_company_id(client, auth_header, data_create_services):
    for data in data_create_services:
        resp = client.post(
            "/api/v1/company/service/", json=data, headers=auth_header
        )

        assert resp.status_code == status.HTTP_201_CREATED

    resp = client.get(
        "/api/v1/company/service/", headers=auth_header
    )
...
async def test_get_service_by_id(client, auth_header, data_create_service):
    ...
    resp = client.get(
        f"/api/v1/company/service/{resp_create_service.json()['id']}", headers=auth_header
    )
...
async def test_get_service_by_id_when_not_found(client, auth_header):
    resp = client.get(
        "/api/v1/company/service/1000", headers=auth_header
    )
