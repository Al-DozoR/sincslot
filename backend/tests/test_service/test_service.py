...
async def test_get_services_by_company_id(client, auth_header, data_create_services):
    for data in data_create_services:
        resp = client.post(
            "/api/v1/service/", json=data, headers=auth_header
        )

        assert resp.status_code == status.HTTP_201_CREATED

    resp = client.get(
        "/api/v1/service/list/", headers=auth_header
    )
...
async def test_create_service(client, auth_header, data_create_service):
    resp = client.post(
        "/api/v1/service/", json=data_create_service, headers=auth_header
    )
...
async def test_get_service_by_id(client, auth_header, data_create_service):
    ...
    resp = client.get(
        f"/api/v1/service/{resp_create_service.json()['id']}", headers=auth_header
    )
...
async def test_get_service_by_id_when_not_found(client, auth_header):
    resp = client.get(
        "/api/v1/service/1000", headers=auth_header
    )

    assert resp.status_code == status.HTTP_404_NOT_FOUND


async def test_update_service_by_id(client, auth_header, data_create_service, data_update_service):
    resp_create_service = client.post(
        "/api/v1/service/", json=data_create_service, headers=auth_header
    )
    ...
    resp_update_service = client.patch(
        f"/api/v1/service/{resp_create_service.json()['id']}", headers=auth_header, json=data_update_service
    )
    ...
    assert resp_update_service.json()["description"] == data_update_service["description"]
