...
@router.patch("/{booking_id}", responses={
...
async def change_booking_schedule_status(
    ...
    if booking_by_id is None:
        return JSONResponse(
            ...
            content=BookingErrorResponse(
                error=f"Booking with id {booking_id} does not exist").model_dump()
        )
    ...
    return JSONResponse(
        ...
        content=CompanyBookingStatusResponse(status=updated_booking.status).model_dump())
