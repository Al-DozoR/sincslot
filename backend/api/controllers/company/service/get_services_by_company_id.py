...
from backend.api.response.service import ServiceErrorResponse, ServiceEntityListResponse, ServiceEntityResponse
...
async def get_services_by_company_id(company=Depends(get_current_company_from_token),
    ...
    try:
        services = await service_use_case.get_services_by_company_id(session, company_id=company.id)
    except Exception as ex:
        logger.error(
            "Error occurred while finding services by company_id %s Error: %s", company.id, str(ex), exc_info=True
        )
        return JSONResponse(
    ...
    response_services = [ServiceEntityResponse(
        id=s.id,
        name=s.name,
        price=s.price,
        duration=s.duration,
    ...
    return JSONResponse(
        ...
        content=ServiceEntityListResponse(services=response_services).model_dump(exclude_none=True, by_alias=True)
    )
