...
async def get_service_by_id(
    ...
    try:
        ...
        logger.error("Error occurred while finding service by id %s Error: %s", service_id, str(ex), exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=ServiceErrorResponse(error=f"Failed to get service by id {service_id}: {str(ex)}").model_dump()
        )
