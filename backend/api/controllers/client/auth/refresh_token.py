...
from backend.api.response.client import ClientErrorResponse
...
@router.post("/refresh-token", responses={
...
async def refresh_tokens(
    ...
    if refresh_token is None:
        ...
        return JSONResponse(
            ...
            content=ClientErrorResponse(error="Refresh token was not provided").model_dump()
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=ClientErrorResponse(error=f"Failed to parse refresh token").model_dump()
        )
    ...
    if not is_refresh:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=ClientErrorResponse(error=f"Provided token is not refresh token").model_dump()
        )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content=ClientErrorResponse(error=f"Failed to update toens").model_dump()
        )
    ...
    if new_tokens is None:
        ...
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=ClientErrorResponse(error=f"Failed to update tokens").model_dump()
        )
