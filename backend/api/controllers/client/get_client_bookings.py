...
async def get_client_bookings(
    ...
    try:
        client_bookings = await booking_use_case.get_booking_by_client(session, client_id=client.id)
    ...
    return JSONResponse(
        ...
        content=json.loads(
            ClientBookingsResponse(client_bookings=client_bookings).model_dump_json(by_alias=True)
        ),
    )
