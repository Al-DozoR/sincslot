...
async def deactivate_client_booking(
    ...
    try:
        ...
        logger.error("Failed to get booking by id %s. Error: %s", booking_id, str(ex), exc_info=True)
        return JSONResponse(
    ...
    try:
        ...
        logger.error("Failed to cancel booking %s. Error: %s", booking_id, str(ex), exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=CompanyErrorResponse(
